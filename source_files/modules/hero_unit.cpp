...
void HeroUnit::changeAttackModule(){
    current_attack_module_number++;
    if (current_attack_module_number == (int)attack_modules.size())
        current_attack_module_number = 0;
    attack_module = attack_modules[current_attack_module_number];
    attack_module->initialize();
}

void HeroUnit::addBullet(Vector direction){
    GameProxy::addHeroUnitBullet(std::make_shared<Bullet>(shape->getFillColor(), position, direction.resize(BULLET_SPEED), damage, attack_range));
}
